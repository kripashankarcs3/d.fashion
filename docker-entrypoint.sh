#!/bin/sh
set -e

# ── Stylist AI: only OPENCODE_MODE=server needs a local `opencode serve`
#    process on loopback. The default mode, "openrouter", is a plain
#    outbound HTTPS call from the app itself and needs none of this — running
#    opencode serve unconditionally here (as this script used to) meant a
#    second heavy process competed for RAM on every boot even when nothing
#    ever talked to it, which is what was exceeding this container's memory
#    limit and triggering repeated restarts.
if [ "$OPENCODE_MODE" = "server" ]; then
  DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/opencode"
  SANDBOX_DIR="${OPENCODE_SERVER_DIRECTORY:-/tmp/deestyle-stylist}"

  mkdir -p "$DATA_DIR" "$SANDBOX_DIR"

  if [ -n "$OPENCODE_AUTH_B64" ]; then
    echo "$OPENCODE_AUTH_B64" | base64 -d > "$DATA_DIR/auth.json" 2>/dev/null || \
      echo "OPENCODE_AUTH_B64 did not decode as base64" >&2
  fi

  echo "[entrypoint] starting opencode serve on 127.0.0.1:4096" >&2
  opencode serve --hostname 127.0.0.1 --port 4096 >&2 &
  SERVE_PID=$!
  trap 'kill "$SERVE_PID" 2>/dev/null || true' EXIT INT TERM

  _ready() {
    node -e "
      const creds = Buffer.from(
        (process.env.OPENCODE_SERVER_USERNAME||'opencode') + ':' +
        (process.env.OPENCODE_SERVER_PASSWORD||'')
      ).toString('base64');
      fetch('http://127.0.0.1:4096/experimental/tool/ids', {
        headers: { Authorization: 'Basic ' + creds },
        signal: AbortSignal.timeout(5000)
      }).then(r => process.exit(r.ok ? 0 : 1)).catch(() => process.exit(1));
    " >/dev/null 2>&1
  }

  i=0
  until _ready; do
    i=$((i + 1))
    if [ "$i" -ge 60 ]; then
      echo "[entrypoint] opencode serve never became ready" >&2
      exit 1
    fi
    sleep 1
  done
  echo "[entrypoint] opencode serve ready" >&2
fi

exec node server/dist/server.js
